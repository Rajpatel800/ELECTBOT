...
export default function HomePage() {
  ...
  return (
    <>
      {/* ====== COUNTRY SELECTOR ====== */}
      <section
        className="max-w-6xl mx-auto px-6 pt-10 pb-2 flex flex-col items-center"
        aria-labelledby="country-selector-heading"
      >
        <div className="text-center mb-4">
          <p
            id="country-selector-heading"
            className="text-sm font-semibold text-[#767683] uppercase tracking-wider mb-3"
          >
            🌍 Select Your Country
          </p>
          <div className="flex gap-3 justify-center flex-wrap" role="group" aria-label="Country selection">
            {countries.map((cn) => (
              <button
                key={cn.code}
                onClick={() => setCountry(cn.code)}
                aria-pressed={country === cn.code}
                aria-label={`Select ${cn.label} — ${cn.sublabel}`}
                className={`relative flex items-center gap-3 px-6 py-4 rounded-2xl border-2 font-bold transition-all duration-200 min-w-[160px] justify-center focus:outline-none focus:ring-2 focus:ring-[#3d4fc8] focus:ring-offset-2 ${
                  country === cn.code
              ...
              >
                <span className="text-3xl" aria-hidden="true">{cn.flag}</span>
                ...
                {country === cn.code && (
                  <span
                    className="absolute -top-2 -right-2 w-6 h-6 rounded-full bg-[#ffc107] text-[#1a237e] text-xs font-black flex items-center justify-center shadow-md"
                    aria-hidden="true"
                  >
                    ✓
                  </span>
        ...
        >
          <span aria-hidden="true">{country === "india" ? "🇮🇳" : "🇺🇸"}</span>
          <span>
            {country === "india"
      ...
      {/* ====== HERO ====== */}
      <section
        className="max-w-6xl mx-auto px-6 py-12 grid grid-cols-1 md:grid-cols-2 gap-10 items-center"
      ...
      >
        ...
        <div>
          ...
          <h1
            id="hero-heading"
            className="text-5xl lg:text-6xl font-black leading-[1.05] tracking-tight text-[#000666] mb-5"
          >
            {c.headline1}
            <br />
          ...
          <div className="flex flex-wrap gap-3 mb-8">
            <Link
              href="/learn"
              className="px-6 py-3.5 rounded-xl bg-gradient-to-r from-[#1a237e] to-[#3d4fc8] text-white font-bold text-base shadow-[0_0_24px_rgba(26,35,126,0.35)] hover:-translate-y-1 hover:shadow-[0_8px_28px_rgba(26,35,126,0.45)] transition-all focus:outline-none focus:ring-2 focus:ring-[#3d4fc8] focus:ring-offset-2"
            >
              ✨ {country === "india" ? "Seekhna Shuru Karein" : "Start Learning"}
            </Link>
            <Link
              href="/chat"
              className="px-6 py-3.5 rounded-xl bg-[#ffdf9e] text-[#785900] font-bold text-base hover:-translate-y-1 hover:bg-[#fdc003] transition-all focus:outline-none focus:ring-2 focus:ring-[#ffc107] focus:ring-offset-2"
            >
              💬 {country === "india" ? "ElectBot se Poochhen" : "Ask ElectBot"}
            </Link>
          </div>
          {/* Stats */}
          <div className="flex items-center gap-6" aria-label="Platform statistics">
            {[c.stat1, c.stat2, c.stat3].map(([n, l], i) => (
              <div key={i} className="flex items-center gap-6">
                {i > 0 && <div className="w-px h-8 bg-[#c6c5d4]" aria-hidden="true" />}
                <div className="text-center">
                  <span className="block text-2xl font-black text-[#1a237e] tracking-tight" aria-label={`${n} ${l}`}>{n}</span>
                  <span className="text-[11px] text-[#767683] font-semibold uppercase tracking-wider" aria-hidden="true">{l}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="hidden md:flex items-center justify-center relative h-[380px]" aria-hidden="true">
      ...
      >
        <div className="text-center mb-10">
          <h2 id="features-heading" className="text-3xl font-black text-[#000666] tracking-tight mb-2">
        ...
        <ul className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5" aria-label="Learning features">
          {feats.map((f) => (
            <li key={f.num}>
              <Link
                href={f.href}
                className="relative bg-white rounded-2xl p-7 shadow-[0_2px_12px_rgba(27,27,33,0.07)] border border-[#c6c5d4]/20 overflow-hidden hover:-translate-y-1 hover:shadow-[0_12px_32px_rgba(27,27,33,0.13)] transition-all group flex flex-col h-full focus:outline-none focus:ring-2 focus:ring-[#3d4fc8] focus:ring-offset-2"
                aria-label={`${f.title}: ${f.desc}`}
              >
                <span className="absolute bottom-3 right-4 text-[4.5rem] font-black text-[#e4e1ea] leading-none select-none pointer-events-none" aria-hidden="true">{f.num}</span>
                ...
                <span className="text-sm font-bold text-[#3d4fc8] group-hover:underline mt-auto">{f.cta}</span>
              </Link>
            </li>
          ))}
        </ul>
      ...
      {/* ====== STEPS ====== */}
      <section
        className="bg-gradient-to-r from-[#1a237e] to-[#3d4fc8] py-14 px-6"
        aria-labelledby="steps-heading"
      >
        <div className="max-w-6xl mx-auto">
          <h2 id="steps-heading" className="text-3xl font-black text-white text-center tracking-tight mb-10">
            {country === "india" ? "Chunav Prakriya — Ek Nazar Mein" : "The Election Process at a Glance"}
          </h2>
          <ol className="flex flex-wrap justify-center items-start gap-4" aria-label="Election process steps">
            {c.steps.map((s, i) => (
              <li key={s.n} className="flex items-start gap-4">
                <div className="flex flex-col items-center text-center min-w-[90px]">
                  <div
                    className="w-12 h-12 rounded-full bg-white/15 border-2 border-white/40 flex items-center justify-center text-xl font-black text-[#ffdf9e] mb-2"
                    aria-hidden="true"
                  >
                    {s.n}
                  </div>
                ...
                {i < c.steps.length - 1 && (
                  <span className="text-white/40 text-2xl mt-3 hidden sm:block" aria-hidden="true">→</span>
                )}
              </li>
            ))}
          </ol>
